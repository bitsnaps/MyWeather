...
class OpenWeatherMapUrlBuilderTest extends Specification {
    ...
    def "test urlForTodaysForecast shall build correct url"() {
        ...
        def decSym = new java.text.DecimalFormat().decimalFormatSymbols.decimalSeparator
        when:
        def result = urlBuilder.urlForTodaysForecast(new LatLng(lat, lon), UnitSystem.IMPERIAL)
        then:
        result == "http://api.openweathermap.org/data/2.5/weather?lat=23${decSym}100000&lon=-12${decSym}100000&appid=" + appid + "&units=imperial"
    }
