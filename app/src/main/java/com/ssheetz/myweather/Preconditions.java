...
@SuppressWarnings({ "unused", "JavadocReference" }) // copy of class from Guava do not want to delete methods we are not currently using
public final class Preconditions {
   ...
   */
    ...
    static String format(@Nullable String template, Object... args) {
        String templateNonNull = String.valueOf(template); // null -> "null"

        // start substituting the arguments into the '%s' placeholders
        StringBuilder builder = new StringBuilder(templateNonNull.length() + 16 * args.length);
        int templateStart = 0;
        int i = 0;
        while (i < args.length) {
            int placeholderStart = templateNonNull.indexOf("%s", templateStart);
            if (placeholderStart == -1) {
                break;
            }
            builder.append(templateNonNull.substring(templateStart, placeholderStart));
            builder.append(args[i++]);
            templateStart = placeholderStart + 2;
        }
        builder.append(templateNonNull.substring(templateStart));
